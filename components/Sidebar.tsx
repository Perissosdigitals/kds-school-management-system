...
const mainNavLinks: { id: Page; icon: string; label: string }[] = [
  ...
  { id: 'documentation', icon: 'bxs-file-doc', label: 'Gestion des Documents' },
];
...
const adminNavLinks: { id: Page; icon: string; label: string }[] = [
  ...
  { id: 'module-management', icon: 'bxs-check-shield', label: 'Gestion des Modules' },
  { id: 'data-management', icon: 'bxs-data', label: 'Gestion des Données' },
  { id: 'reports', icon: 'bxs-report', label: 'Rapports' },
  { id: 'activity-log', icon: 'bx-history', label: 'Journal d\'Activités' },
];
...
const NavLink = React.memo<{
  ...
  <a
    ...
    className={`flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-all duration-300 ${isActive ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-700 hover:bg-emerald-50 hover:text-emerald-700'
      }`}
  >
...
export const Sidebar: React.FC<SidebarPropsExtended> = React.memo(({ activePage, setActivePage, user, isOpen = true, onClose }) => {

  const filteredMainNavs = mainNavLinks.filter(link => hasPermission(user, link.id));
  const filteredAdminNavs = adminNavLinks.filter(link => hasPermission(user, link.id));
  ...
  return (
    <>
      ...
      <nav className={`
        ...
        w-64 glass-panel p-6 
        flex flex-col flex-shrink-0
        transform transition-transform duration-300 ease-in-out
        ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
      `}>
        <div className="flex items-center gap-3 mb-12 text-emerald-600">
