...
echo "🚀 Starting Class Cleanup V2..."

# 1. Migrate students from CM2 Test to CM2-A
echo "📋 Fetching students from CM2 Test..."
# Corrected endpoint: Use /students?classId=...
STUDENTS=$(curl -s "$API_URL/students?classId=$CM2_TEST_ID" | jq -r '.data[] | .id')

if [ -z "$STUDENTS" ]; then
    echo "ℹ️  No students found in CM2 Test (or API returned empty)."
else
    echo "🔄 Migrating students to CM2-A..."
    for STUDENT_ID in $STUDENTS; do
        echo "  - Moving student $STUDENT_ID..."
        # Note: The endpoint to update student class might be PUT /students/:id or PATCH /students/:id
        # Based on API_ENDPOINTS.md, it's PUT /api/v1/students/:id for partial update
